...
class Transaction(BaseModel):
    ...
    type: str | None = Field(
        ..., description="The type of the transaction, e.g., debit or credit"
    )
    description: str = Field(..., description="A brief description of the transaction")
    category: str | None = Field(
        ..., description="The category of the transaction, e.g., groceries, utilities"
    )
...
class CreditCardBill(BaseModel):
    ...
    file_hash: str = Field(
        ..., description="A unique hash of the file for verification purposes"
    )
    bank_name: str = Field(
    ...
    reference_month: str = Field(
        ...,
        description='The reference month for the credit card bill, formatted as "YYYY-MM"',
    )
    ...
    transactions: list[Transaction] = Field(
        ..., description="A list of transactions included in the bill"
    )
...
class BankStatement(BaseModel):
    ...
    bank_name: str = Field(
        ..., description="The name of the bank issuing the statement"
    )
    start_date: datetime.date = Field(
    ...
    transactions: list[Transaction] = Field(
        ..., description="A list of transactions included in the statement"
    )
